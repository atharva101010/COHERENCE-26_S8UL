...
export default function AIAgentNode({ data, selected }) {
  ...
  return (
    <div className={`px-4 py-3 rounded-xl border-2 min-w-[190px] shadow-md transition-all
      ${selected ? 'border-purple-500 shadow-purple-200 ring-2 ring-purple-300' : 'border-purple-300'}
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-xs font-medium text-purple-600 uppercase tracking-wide">AI Agent</div>
          <div className="text-sm font-semibold text-purple-900">{data.label || 'AI Agent'}</div>
        </div>
      </div>
      <div className="mt-2 flex items-center gap-1.5 flex-wrap">
        <span className="text-[10px] font-medium bg-purple-200/60 text-purple-700 px-1.5 py-0.5 rounded">
          {provider.toUpperCase()}
        </span>
        {data.model && (
          <span className="text-[10px] text-purple-500 truncate max-w-[120px]">{data.model}</span>
        )}
      </div>
