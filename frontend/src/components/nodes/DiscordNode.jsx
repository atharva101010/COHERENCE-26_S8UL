  ...
  return (
    <div className={`px-4 py-3 rounded-xl border-2 min-w-[170px] shadow-md transition-all
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-xs font-medium text-indigo-600 uppercase tracking-wide">Discord</div>
          <div className="text-sm font-semibold text-indigo-900">{data.label || 'Discord Message'}</div>
        </div>
      </div>
      {data.webhookUrl && (
        <div className="mt-2 text-xs text-indigo-700 bg-indigo-200/50 px-2 py-0.5 rounded-full w-fit truncate max-w-[160px]">
          Webhook configured
        </div>
      )}
