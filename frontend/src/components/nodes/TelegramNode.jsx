...
export default function TelegramNode({ data, selected }) {
  return (
    <div className={`px-4 py-3 rounded-xl border-2 min-w-[170px] shadow-md transition-all
      ...
      bg-gradient-to-br from-sky-50 to-blue-100 dark:from-sky-950/30 dark:to-blue-900/30`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-xs font-medium text-sky-600 dark:text-sky-400 uppercase tracking-wide">Telegram</div>
          <div className="text-sm font-semibold text-sky-900 dark:text-sky-100">{data.label || 'Telegram Message'}</div>
        </div>
      </div>
      {data.chatId && (
        <div className="mt-2 text-xs text-sky-700 dark:text-sky-300 bg-sky-200/50 dark:bg-sky-800/40 px-2 py-0.5 rounded-full w-fit truncate max-w-[160px]">
          Chat: {data.chatId}
        </div>
      )}
