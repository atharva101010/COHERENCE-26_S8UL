...
export default function FilterNode({ data, selected }) {
  return (
    <div className={`px-4 py-3 rounded-xl border-2 min-w-[170px] shadow-md transition-all
      ...
      bg-gradient-to-br from-indigo-50 to-blue-100 dark:from-indigo-950/30 dark:to-blue-900/30`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-xs font-medium text-indigo-600 dark:text-indigo-400 uppercase tracking-wide">Filter</div>
          <div className="text-sm font-semibold text-indigo-900 dark:text-indigo-100">{data.label || 'Filter'}</div>
        </div>
      </div>
      {data.rules && data.rules.length > 0 && (
        <div className="mt-2 text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-200/50 dark:bg-indigo-800/40 px-2 py-0.5 rounded-full w-fit">
